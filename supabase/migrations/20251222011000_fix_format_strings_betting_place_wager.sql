-- ============================================================================
-- Fix invalid format strings in betting_place_wager (stake guard messages)
-- ============================================================================
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Pool not found for id %s', p_market_id;
  END IF;

  SELECT * INTO container_row FROM public.events WHERE id = pool_row.event_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Event not found for pool %s', pool_row.id;
  END IF;

  IF pool_row.status <> 'open' THEN
    RAISE EXCEPTION 'Pool %s is not open (status=%s)', pool_row.name, pool_row.status;
  END IF;

  IF now() > COALESCE(pool_row.close_time, now() + interval '100 years') THEN
    RAISE EXCEPTION 'Pool %s is closed for wagering (closed at %s)', pool_row.name, pool_row.close_time;
  END IF;
  ...
  IF p_stake < normalized_min THEN
    RAISE EXCEPTION USING
      errcode = 'P0001',
      message = format(
        'Stake Ɖ%s is below the minimum Ɖ%s allowed for pool "%s" (%s)',
        p_stake,
        normalized_min,
        pool_row.name,
        pool_row.id
  ...
  IF normalized_max IS NOT NULL AND p_stake > normalized_max THEN
    RAISE EXCEPTION USING
      errcode = 'P0001',
      message = format(
        'Stake Ɖ%s exceeds the maximum Ɖ%s allowed for pool "%s" (%s)',
        p_stake,
        normalized_max,
        pool_row.name,
        pool_row.id
