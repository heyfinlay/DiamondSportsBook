-- ============================================================================
-- CONTEXTUAL MARKET TEMPLATES + F1 SUPPORT
-- Improve generated market naming/copy and make template matching resilient.
-- ============================================================================

UPDATE public.sports_market_templates
SET config = COALESCE(config, '{}'::jsonb) || jsonb_build_object(
  'sort_order', 10,
  'market_name_template', '{{event_title}} Winner',
  'market_description_template', 'Parimutuel pool on the winning driver in {{event_title}}.'
)
WHERE market_key = 'f1_race_winner';

UPDATE public.sports_market_templates
SET config = COALESCE(config, '{}'::jsonb) || jsonb_build_object(
  'sort_order', 20,
  'market_name_template', '{{event_title}} Top 3 Finish',
  'market_description_template', 'Parimutuel pool on whether a driver finishes on the podium in {{event_title}}.'
)
...
SET config = COALESCE(config, '{}'::jsonb) || jsonb_build_object(
  'sort_order', 10,
  'market_name_template', '{{home_name}} vs {{away_name}} Winner',
  'market_description_template', 'Parimutuel pool on the winner of {{home_name}} vs {{away_name}}.'
)
WHERE market_key = 'nrl_match_winner';

CREATE OR REPLACE FUNCTION public.sports_apply_market_template_text(
  p_template text,
  p_context jsonb
)
RETURNS text
...
BEGIN
  rendered := replace(rendered, '{{event_title}}', COALESCE(p_context->>'event_title', ''));
  rendered := replace(rendered, '{{competition_name}}', COALESCE(p_context->>'competition_name', ''));
  rendered := replace(rendered, '{{round_label}}', COALESCE(p_context->>'round_label', ''));
  rendered := replace(rendered, '{{venue_name}}', COALESCE(p_context->>'venue_name', ''));
  rendered := replace(rendered, '{{home_name}}', COALESCE(p_context->>'home_name', 'TBD'));
  rendered := replace(rendered, '{{away_name}}', COALESCE(p_context->>'away_name', 'TBD'));
  rendered := regexp_replace(rendered, '\s+', ' ', 'g');
  RETURN NULLIF(btrim(rendered), '');
END;
$$;
