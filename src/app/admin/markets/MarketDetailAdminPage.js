...
import { closePool, confirmSettlement, fetchAdminMarketDetail, fetchMarketWagers, fetchWalletActivityForMarket, fetchRakeLedger, openPool, previewSettlement, proposeSettlement, suspendPool, voidPool, archivePool, restorePool, updatePoolCopy } from "@domains/betting/api/marketAdminApi";
...
const PoolConfigModal = ({ pool, onClose, onSuccess }) => {
    const { toast } = useToast();
    const [name, setName] = useState(pool.label);
    const [description, setDescription] = useState(pool.description ?? "");
    const updateMutation = useMutation({
        mutationFn: () => updatePoolCopy(pool.id, {
            name,
            label: name,
    ...
    return (_jsx("div", { className: "fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4", children: _jsxs("div", { className: "w-full max-w-2xl rounded-3xl border border-white/10 bg-[#060910] p-8 shadow-2xl", children: [_jsxs("header", { className: "flex items-start justify-between", children: [_jsxs("div", { children: [_jsx("p", { className: "text-xs uppercase tracking-[0.35em] text-white/50", children: "Pool Configuration" }), _jsx("h2", { className: "text-2xl font-semibold text-white", children: "Edit Pool Copy" }), _jsx("p", { className: "mt-1 text-sm text-white/60", children: "Update display name and description (settlement logic unchanged)" })] }), _jsx("button", { type: "button", onClick: onClose, className: "text-xs uppercase tracking-[0.3em] text-white/60 transition hover:text-white", children: "Close" })] }), _jsxs("form", { onSubmit: handleSubmit, className: "mt-6 space-y-4", children: [_jsxs("div", { children: [_jsxs("label", { className: "text-xs uppercase tracking-[0.3em] text-white/50", children: ["Pool Name ", _jsx("span", { className: "text-red-400", children: "*" })] }), _jsx("input", { type: "text", value: name, onChange: (e) => setName(e.target.value), className: "mt-2 w-full rounded-2xl border border-white/10 bg-black/40 px-4 py-3 text-white placeholder:text-white/30 focus:border-brand focus:outline-none", placeholder: "e.g. Overall Winner", required: true }), _jsx("p", { className: "mt-1 text-xs text-white/50", children: "This is the primary label shown on cards and market details" })] }), _jsxs("div", { children: [_jsx("label", { className: "text-xs uppercase tracking-[0.3em] text-white/50", children: "Description" }), _jsx("textarea", { value: description, onChange: (e) => setDescription(e.target.value), className: "mt-2 w-full rounded-2xl border border-white/10 bg-black/40 px-4 py-3 text-white placeholder:text-white/30 focus:border-brand focus:outline-none", placeholder: "Optional tagline or helper text", rows: 3 }), _jsx("p", { className: "mt-1 text-xs text-white/50", children: "Displayed below the pool name as supplementary information" })] }), _jsxs("div", { className: "rounded-2xl border border-white/10 bg-white/5 p-4", children: [_jsx("p", { className: "text-xs font-semibold uppercase tracking-[0.3em] text-[#9FF7D3]", children: "Current Pool Info" }), _jsxs("div", { className: "mt-2 grid gap-2 text-sm", children: [_jsxs("p", { className: "text-white/60", children: [_jsx("span", { className: "text-white/40", children: "Type:" }), " ", pool.pool_type] }), _jsxs("p", { className: "text-white/60", children: [_jsx("span", { className: "text-white/40", children: "Status:" }), " ", pool.status] }), _jsxs("p", { className: "text-white/60", children: [_jsx("span", { className: "text-white/40", children: "Total Pool:" }), " ", `${currencySymbol}${pool.total_pool.toLocaleString()}`] }), _jsxs("p", { className: "text-white/60", children: [_jsx("span", { className: "text-white/40", children: "Rake:" }), " ", (pool.rake_percent * 100).toFixed(1), "%"] })] })] }), _jsxs("div", { className: "flex gap-3 pt-4", children: [_jsx("button", { type: "submit", disabled: updateMutation.isPending, className: "flex-1 rounded-2xl bg-brand px-6 py-3 text-sm font-semibold uppercase tracking-[0.3em] text-black disabled:opacity-40", children: updateMutation.isPending ? "Saving…" : "Save Changes" }), _jsx("button", { type: "button", onClick: onClose, className: "rounded-2xl border border-white/20 px-6 py-3 text-sm font-semibold uppercase tracking-[0.3em] text-white transition hover:bg-white/5", children: "Cancel" })] })] })] }) }));
};
