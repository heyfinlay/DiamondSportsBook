...
const MarketsPage = () => {
    ...
    const eventsWithActiveMarkets = eventsQuery.data?.filter((event) => event.markets.length > 0) ?? [];
    const hasEvents = eventsWithActiveMarkets.length > 0;
    return (_jsxs("div", { className: "flex flex-col gap-10", children: [_jsxs("header", { className: "flex flex-col gap-4 rounded-3xl border border-white/5 bg-[#060910]/80 p-8 shadow-[0_0_40px_rgba(15,23,42,0.45)]", children: [_jsx("span", { className: "text-xs uppercase tracking-[0.35em] text-[#9FF7D3]", children: "Diamond Sports Book" }), _jsx("h1", { className: "text-4xl font-semibold text-white sm:text-5xl", children: "Live Markets" }), _jsx("p", { className: "max-w-2xl text-sm text-neutral-300 sm:text-base", children: "All DBGP betting uses a live parimutuel system. Your payout depends on the total Diamonds staked across each outcome." }), _jsxs("div", { className: "rounded-2xl border border-white/10 bg-white/5 p-4", children: [_jsx("p", { className: "text-xs font-semibold uppercase tracking-[0.3em] text-[#9FF7D3]", children: "How It Works" }), _jsx("p", { className: "mt-2 text-sm text-neutral-300", children: "Markets update in real time as Diamonds move across the pool. Odds and payout estimates will rise or fall until the market closes and locks your final price." })] }), _jsx("p", { className: "text-[0.7rem] uppercase tracking-[0.3em] text-neutral-500", children: "All wagers settled in Diamonds (in-game currency). Parody product; no real-world stakes." })] }), eventsQuery.isLoading && (_jsx("p", { className: "text-sm text-neutral-400", children: "Loading live markets\u2026" })), eventsQuery.isError && (_jsx("p", { className: "rounded-2xl border border-red-400/30 bg-red-500/10 px-4 py-3 text-sm text-red-200", children: "Unable to load markets right now. Refresh to try again." })), hasEvents ? (_jsx("section", { className: "flex flex-col gap-6", children: eventsWithActiveMarkets.map((event) => (_jsxs("div", { className: "flex flex-col gap-4 rounded-3xl border border-white/5 bg-[#05070F]/80 p-6", children: [_jsxs("header", { className: "flex flex-col gap-1", children: [_jsx("span", { className: "text-xs uppercase tracking-[0.35em] text-[#7C6BFF]", children: "Event" }), _jsx("h2", { className: "text-2xl font-semibold text-white", children: event.title }), _jsxs("p", { className: "text-sm text-neutral-400", children: [event.session?.track_name ? `${event.session.track_name} • ` : "", event.starts_at ? new Date(event.starts_at).toLocaleString() : "Schedule TBC"] })] }), _jsx("div", { className: "grid gap-4 md:grid-cols-2", children: event.markets.map((market) => (_jsxs("article", { className: "flex flex-col gap-3 rounded-2xl border border-white/10 bg-[#060910]/80 p-4", children: [_jsxs("div", { className: "flex items-center justify-between gap-3", children: [_jsx("h3", { className: "text-[1.125rem] font-semibold leading-tight text-white md:text-[1.35rem]", children: market.name }), _jsx("span", { className: "shrink-0 text-xs uppercase tracking-[0.35em] text-neutral-500", children: formatStatus(market.status) })] }), _jsx("div", { children: market.description && (_jsx("p", { className: "text-sm text-neutral-400", children: market.description })) }), _jsxs("p", { className: "text-xs text-neutral-400", children: [formatClosesAt(market.close_time), " \u2022 Pool \u0189", market.total_pool.toLocaleString()] }), _jsxs("ul", { className: "flex flex-wrap gap-2 text-xs text-neutral-300", children: [market.outcomes.slice(0, 4).map((outcome) => {
                                                ...
                                                return (_jsx("li", { className: "min-w-[10rem]", children: _jsxs("button", { type: "button", className: "flex w-full items-center gap-2 rounded-2xl border border-white/10 px-3 py-2 text-left transition hover:border-brand hover:text-white", onClick: () => handleOutcomeSelect(event, market, outcome), children: [_jsx("span", { className: "h-2.5 w-2.5 rounded-full", style: { backgroundColor: identity.color } }), _jsxs("span", { className: "flex flex-col text-xs", children: [_jsx("span", { className: "font-semibold text-white", children: identity.primaryLabel }), identity.secondaryLabel && (_jsx("span", { className: "text-[11px] text-neutral-400", children: identity.secondaryLabel }))] })] }) }, outcome.id));
                                            }), market.outcomes.length > 4 && (_jsxs("li", { className: "rounded-full border border-white/10 px-3 py-1 text-neutral-500", children: ["+", market.outcomes.length - 4, " more"] }))] }), _jsx(Link, { to: `/market/${market.id}`, className: "text-xs font-semibold uppercase tracking-[0.35em] text-[#9FF7D3] transition hover:text-white", children: "View details \u2192" })] }, market.id))) })] }, event.id))) })) : (!eventsQuery.isLoading && (_jsxs("div", { className: "flex flex-wrap items-center gap-3 rounded-3xl border border-dashed border-white/10 bg-[#05070F]/40 p-8 text-sm text-neutral-400", children: [_jsxs("div", { className: "flex-1", children: [_jsx("p", { className: "font-semibold text-white", children: "Live market board coming online" }), _jsx("p", { children: "Admin tools will seed the first tote shortly. Check back once the next event opens betting." })] }), _jsx(Link, { to: "/account", className: "inline-flex items-center gap-2 rounded-full border border-[#9FF7D3]/40 px-4 py-2 uppercase tracking-[0.35em] text-[#9FF7D3] transition hover:border-[#9FF7D3]/70 hover:text-white", children: "Manage wallet" })] })))] }));
};
