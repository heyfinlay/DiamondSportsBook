...
export const TrackStatusBanner = ({ status, variant = "live" }: TrackStatusBannerProps) => {
  ...
  return (
    <div
      className={`rounded-3xl border px-6 py-4 shadow-lg shadow-black/30 ${config.className}`}
      role="status"
      aria-live="polite"
    >
      <p className="text-xs uppercase tracking-[0.35em] opacity-80">{config.title}</p>
      <p className={`${sizeClasses} font-semibold`}>{config.description}</p>
    </div>
  );
};
