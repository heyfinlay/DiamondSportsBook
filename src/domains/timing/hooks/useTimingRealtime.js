...
export const useTimingRealtime = (sessionId) => {
    ...
    useEffect(() => {
        if (!sessionId)
            return;
        const channel = subscribeToChannel(`session-${sessionId}`, { config: { broadcast: { ack: true } } })
            ...
            .on("postgres_changes", { event: "*", schema: "public", table: "live_driver_standings", filter: `session_id=eq.${sessionId}` }, (payload) => {
            const rows = Array.isArray(payload.new) ? payload.new : [payload.new];
            upsertDrivers(rows.map((row) => ({
                id: row.driver_id,
            ...
            .on("postgres_changes", { event: "INSERT", schema: "public", table: "laps", filter: `session_id=eq.${sessionId}` }, (payload) => {
            pushLapEvent({
                id: payload.new.id,
