...
4. **Manual Leaderboard**
   ...
   - Admins can toggle a manual override, set custom points/positions, and persist them directly on the `championship_drivers` / `championship_teams` rows.
...
- The `championship_results` rows carry `display_label_mode` while manual leaderboard overrides now live on the `championship_drivers` and `championship_teams` tables themselves (`manual_points`, `manual_position`, `use_manual_override`), keeping all standings data in one place with admin-only update access.
